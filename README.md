...
# ASSOC
This package also includes a library that manages (for the time being, in a pedestrian way...) a _class_ of associative arrays. This is similar to, e.g., Python dictionaries. To use it, the user has to include the _assoc.h_ header file and link the _libassoc.a_ library. An example of usage is available in file _atest.c_
...
Just compile atest with

    gcc -o atest atest.c libassoc.a

There is also an acgi.c code that can be used to create associative arrays from HTML forms!
